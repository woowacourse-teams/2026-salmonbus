...
### 13.3 두 컨텍스트가 같은 수를 쓸 때

같은 숫자가 두 컨텍스트에 나오면 먼저 **그것이 하나의 사실인지 각자의 결정인지** 가른다.
각자의 결정이면 `common`으로 올려 공유하지 않는다. 공유 커널로 묶는 순간 두 컨텍스트가 함께 바뀌어야 하고,
값을 바꾸는 일이 두 모듈 수정과 배포 순서 규칙이 된다.

관측 신선도 창이 그 예다. 예보는 "어떤 판에 계산할 값어치가 있는가"를 정하고 조회는 "발행된 판을 언제까지
보여줄 것인가"를 정한다. 두 컨텍스트를 잇는 것은 숫자가 아니라 이미 **발행 사실**이다. 보드는 발행이 있는
판만 고른다. 그래서 값을 합치는 대신 **방향 있는 불변식**(예보 창 ≥ 조회 창)을 드러내고, 소비자 쪽 계약
테스트가 빌드에서 그 관계를 고정한다. 설정으로 어긋날 수 있는 경로는 기동에서 막는다.

### 13.4 발행 식별 정보와 재현 범위
