...
따라서 `gbis-client`는 HTTP·인증·JSON·외부 오류를 다루고, 카탈로그와 관측의 변환 어댑터가 각각 노선 정보와 관측 결과를 만든다. GBIS DTO와 Jackson 의존은 도메인에 전달하지 않는다. 하나의 공용 어댑터가 두 업무의 모든 규칙까지 소유하지 않도록 한다.
...
Gradle 모듈을 나눠도 Java public 클래스에는 접근할 수 있으므로 ArchUnit 검사도 필요하다. 공개 패키지 외 참조, 업무 영역 사이의 순환, 도메인의 외부 기술 의존을 검사하되 같은 계층 내부의 정상 호출은 허용한다.
...
- 도착 후보는 원 관측 시각이 아니라 **예보 생성 시각 이후**에서 찾는다.
...
1. 비어 있지 않은 차량 ID의 잔여석이 현재 지원 상한 70을 넘으면 관측 저장과 같은 TX에서 조사 요청을 등록한다. 같은 차량의 진행 중인 조사를 중복 등록하지 않는다.
...
5. 이상 근거 이후의 유효 관측에서 다음 사용 가능 편도가 확인되고 처리할 페이지도 소진돼야 조사를 완료한다. 현재 읽을 자료가 없다는 이유만으로 보류를 풀지 않는다.
...
좌석 상한과 DB 품질 view의 조건도 함께 검증한다. Java 상수만 바꾸고 SQL에 남은 70을 그대로 두면 계산과 조회의 사용 기준이 달라진다. 기본 10분 기준의 운영 적합성이나 이미 끝난 잘못된 조사의 재등록은 이번 PR에서 자동 해결된 것으로 보지 않는다.
