...
- 편도 방향 판정의 조회 view가 첫 정류장을 `stop_order = 1`로 고정한다. 도메인은 판본의 최소 순번을 쓴다. 최소 순번이 1이 아닌 판본이 있어야 값이 갈리며, 그런 판본이 없으면 고쳐도 바뀌는 행이 없다. 다만 최소 순번이 1이라는 보장은 코드·제약·검증 어디에도 없다. 상류 `stationSeq`를 그대로 넣고 `route_stop`에 CHECK가 없으며 `RouteStop`의 검증은 순번이 1 이상인지만 본다.
- 정원 분모가 서빙과 집계에서 다르다. 앞서 `GREATEST(..., 1)`을 원인으로 적었으나 그 경로는 발동하지 않는다. 집계가 예보가 존재하는 라벨만 세고, 예보가 났다는 것은 그 시점 서빙 정원이 0보다 컸다는 뜻이다. 실제로 갈리는 것은 자르는 시각이다. 서빙은 그 예보의 배치까지, 집계는 갱신 시각까지 본다. 두 규칙은 각각 테스트가 못 박고 있어(`JdbcStopDemandStatisticsRepositoryTest`의 `정원은_그_차량이_보여_준_최대_잔여석이다`) 어느 쪽으로 맞추든 통과하던 테스트가 깨진다. 학습 라벨의 정의를 바꾸는 일이라 계산 규칙 버전 상향과 모델 번들 재배포가 함께 간다.
...
승인된 계획은 `gbis-client`를 "HTTP·인증·JSON·GBIS 오류와 원시 응답 담당, 업무 의미로의 변환은 각 업무 모듈 어댑터"로 정했다. 위치정보는 그 계약을 지켰지만 노선정보는 아니었다. 경로 상수 둘, 결과 코드 판정, JSON 역직렬화가 route-catalog의 `GbisRouteSource`에 있었다.

- `gbis-client`에 `GbisRouteInfoSource`와 `GbisRouteResult`를 두어 통신과 응답 해석을 가져갔다. 위치정보의 `GbisLocationSource`·`GbisLocationResult`와 같은 모양이다.
- route-catalog의 `GbisRouteSource`는 회차 순번 판정과 정류소·시간표 구성만 남겨 221행에서 154행이 됐다. 업무 판정은 그대로다.
