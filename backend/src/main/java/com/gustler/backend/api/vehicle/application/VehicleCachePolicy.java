...
public class VehicleCachePolicy {
    ...
    /**
     * max-age 는 응답을 받은 시점부터 세는 값이라 관측 시각이 아니라 응답 시각으로 구간을 정한다.
     */
    public Duration maxAgeAt(OffsetDateTime respondedAt) {
        final int hour = respondedAt.atZoneSameInstant(SEOUL).getHour();
