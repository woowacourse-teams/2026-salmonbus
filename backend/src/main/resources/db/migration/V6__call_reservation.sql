...
--
-- 번호가 6 인 것은 브랜치 머지 순서를 따라간 것이다. 궤적(V5)이 이 브랜치보다 먼저 머지된다.
-- 5 를 쓰면 궤적이 들어간 dev 를 한 번이라도 띄운 DB 에 이 파일이 낮은 번호로 뒤늦게 도착하고,
-- Flyway 기본값(outOfOrder=false)이 그것을 안 돌리고 검증에서 막는다.
-- MigrationOrderTest 가 그 순서를 지킨다.
