...
완료한다. 그 뒤 배차가 뜸한 구간에 writer fence를 올리고 activation-last 순서로 임시 모델을
`v41b-8194bde56d86f365afd6`로 교체한다.
...
| 범위 | 정본 | 관계 |
|---|---|---|
| 전체 실행 순서 | 이 문서 | activation-last 운영 순서와 승인 gate |
...
있으므로 cleanup 불변식은 pause 때 고정된 `observation_batch_high_water` 이하만 비교한다.

## fence 의미

`temp-pause`는 advisory lock `(1920224641, 4041)`을 exclusive로 잡아 이미 시작한 shared writer를 drain한
뒤 `forecast_cutover_control.writes_paused=true`와 DB 시계의 `FINAL_CUTOVER_AT`, observation high-water를
한 transaction에서 고정한다. worker의 `JdbcForecastWriteBarrier`는 blocking lock이 아니라
`pg_try_advisory_xact_lock_shared`를 사용한다. lock을 얻지 못하거나 pause flag가 켜져 있으면 해당 파생
write cycle을 즉시 건너뛴다.

| 동작 | pause 중 결과 | 코드 근거 |
|---|---|---|
| raw collection과 `observation_batch`/`vehicle_observation` INSERT | 계속 | `CollectionConfig`의 전용 collection scheduler; collector는 barrier를 호출하지 않음 |
| forecast 생성·`forecast_completed_at` 갱신 | cycle skip, write 없음 | `ForecastBatchWriter.writeForecastsOf` |
| arrival label settle | cycle skip, write 없음 | `JdbcSeatForecastRepository.settle` |
| stop-demand 통계 append | cycle skip, write 없음 | `JdbcStopDemandStatisticsRepository.append` |
| route binding, import, seed, cleanup, model activation | worker barrier 대상 아님 | 각각 migration/activation 승인과 자체 transaction 사용 |

skip은 `/actuator/info`의 `forecastWriteBarrier.skippedCycles`, `consecutiveSkippedCycles`, `lastSkippedAt`,
`lastSkipReason`에 누적된다. 연속 6회마다 WARN이다. DB 확인 오류도 fail-closed skip이다. control table이 아직
없는 선행 worker 배포 구간만 호환을 위해 enter=true다.

board API는 raw 최신 batch가 아니라 `forecast_completed_at`이 있는 최신 batch를 읽는다
(`JpaBoardQueryRepository.findLatestForecastCompleted`). freshness 창은 5분이다. 따라서 pause 직후에는 기존
완료 batch로 200일 수 있지만 약 5분 뒤에는 수집이 계속되어도 `NO_RECENT_OBSERVATION` 503이 정상이다.
unpause 뒤 backlog 처리와 fresh forecast가 끝나야 200이 회복된다.
...
worker 배포 후 historical Flyway V1–V3가 아직 적용되지 않아도 control table 부재 호환 경로로 평소처럼
작동한다. public backfill 전에 반드시 schema를 적용해 `ingestion_origin`을 만들고 positive-only cache가
LIVE queue 분리를 인지했는지 확인한다.
...
| D4. additive schema | `ACADEMY_SCHEMA`, 5–10분 | 별도 history의 historical V1–V3, control unpaused, origin default/index | 진행 중단·증거 보존 |
...
| D6. reconciliation | read-only, 10–20분 | 149,193 batches, 2,461,308 observations, LIVE 불변, source/live seam 무중복 | pause 시작 금지 |

import에는 fence가 필요 없다. importer는 `S3_BACKFILL`, collector JPA INSERT는 default `LIVE`이고 pending
forecast query는 LIVE literal만 읽는다. import 중 collection·forecast·board는 계속된다. route `valid_from`
확장은 exact current version gate 아래 단일 transaction이며 별도 DB write 승인 대상이다.
...
- cleanup, seed apply, one-shot promotion, unpause 승인 담당자가 pause 내내 응답 가능하다.
- 두 route raw observation count가 증가하며 `/actuator/info` skip baseline이 기록됐다.
- 141분 운영 buffer를 확보하지 못하면 pause를 시작하지 않는다.

## activation-last pause 구간
...
이후에는 17:00 전 buffer가 부족하므로 새 pause를 시작하지 않는다.

| 단계 | 승인·예상 | 반드시 남길 증거 | 실패 복구 |
|---|---|---|---|
| P1. `temp-pause` | `ACADEMY_TEMP_CLEANUP`, 2–5분 | pause=true, T/high-water, shared writer drain, raw count 계속 증가, skip counter 증가 | temp sole ACTIVE면 승인된 recovery-unpause |
| P2. freeze dry-run/apply | apply에 cleanup approval, 3–8분 | 같은 T/high-water, exact frozen generation set/SHA | fence 유지, cleanup 금지 |
| P3. cleanup dry-run | read-only, 3–8분 | exact temp forecast/cell target, observation invariant | 불일치 시 DELETE 금지 |
| P4. bounded cleanup+no-op | dry-run SHA에 묶인 별도 deletion 승인, 10–30분 | deleted=target, no-op 0, observations/markers/lineage 보존 | fence 유지, receipt-bound resume |
...
| P8. formal one-shot promotion | 별도 promotion/restart 승인, 8–15분 | true 1회 재기동, formal sole ACTIVE, id 1 RETIRED, 즉시 false 복원 | fence 유지; 별도 temp re-promotion 승인 |
...
예상 pause는 **47–141분**이다. production 246만 관측 full replay는 EC2에서 아직 계측되지 않아 10–35분
범위가 추정치다. 256MiB는 코드가 강제하거나 production에서 입증한 ceiling이 아니라 외부 관측용 stop
threshold다. 35분 또는 256MiB를 넘으면 자동으로 계속하지 말고 fence를 유지한 채 중단 여부를 결정한다.
...
<APPROVED_ATOMIC_CURRENT_SWAP_AND_MODEL_BUNDLE_PROMOTE_ON_START_TRUE_RESTART>
<VERIFY_FORMAL_ACTIVE_AND_RESTORE_MODEL_BUNDLE_PROMOTE_ON_START_FALSE>
...
로그에 남기지 않는다. `seed-rollback --execute true`는 formal activation 전, temp sole ACTIVE, fence 유지
상태에서만 별도 `ACADEMY_SEED_ROLLBACK` 승인으로 실행한다.
...
| activation-last | P1–P10 및 DB state gate | 닫힘 |
...
않고 fence 유지·별도 temp promotion” 규칙으로 위험을 수용한다.

## 실패 복구

자동 finally나 watchdog은 없다. process/SSH가 끊겨도 `writes_paused=true`는 durable하게 남으므로 actuator
skip counter와 raw observation 증가를 감시하고 반드시 승인된 명령으로 닫는다.
...
| formal ACTIVE 이후 검증 실패 | fence 유지, 별도 승인으로 temp bundle을 startup promotion 경로로 재승격 | `--recovery true`, DB 수동 state UPDATE |
...
낮에 pause를 시작하지 못하면 집에서는 SSH가 불가하므로 실행하지 않는다. 밤 창을 쓰려면 학원 SSH 또는
별도 승인된 SSM/보안그룹 접근, operator 2명, 141분+30분 buffer, 모든 artifact/approval이 필요하다.
import와 archive 전송은 낮에 끝내고 P1–P10만 수행한다. 첫차 04:50 전에 board 회복을 확인하려면 늦어도
...
- pause/freeze/cleanup dry-run·execute·no-op receipts;
- seed plan/apply receipt와 final `v4-1-seed-cutover-receipt-v1` receipt;
- one-shot promotion/false 복원 receipt와 formal serving-reference 검증;
- pause 중 raw observation 증가와 actuator skip counter, unpause 뒤 두 route board 200/freshness;
