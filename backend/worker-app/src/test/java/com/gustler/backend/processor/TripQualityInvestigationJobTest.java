...
class TripQualityInvestigationJobTest {
    ...
    void 잠금_시간_초과는_다음_실행으로_미루고_같은_실행에서_반복하지_않는다() {
        ...
        // when
        assertThatCode(new TripQualityInvestigationJob(repository)::investigate).doesNotThrowAnyException();
    ...
    void 잠금_시간_초과가_아닌_DB_오류는_그대로_전파한다() {
        ...
        // when & then
        assertThatThrownBy(new TripQualityInvestigationJob(repository)::investigate).isSameAs(failure);
    }
}
