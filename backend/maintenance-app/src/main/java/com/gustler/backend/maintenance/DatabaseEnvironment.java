...
public record DatabaseEnvironment(
    ...
    public static DatabaseEnvironment load(
        ...
        try {
            ...
            throw new MaintenanceException("CONFIG_INVALID_TARGET_KIND", e);
        }
        ...
        if (targetKind == TargetKind.LOCAL && !isLoopback(identity.getHost())) {
            throw new MaintenanceException("LOCAL_TARGET_MUST_BE_LOOPBACK");
        }
    ...
    private static void requireAcademyEnvironmentFile(
        ...
        if (!ACADEMY_ENV.equals(path)) {
            throw new MaintenanceException("ACADEMY_ENV_FILE_PATH_INVALID");
        }
        SecureFiles.requirePrivateRegularFile(path);
        try {
            UserPrincipal owner = Files.getOwner(path, LinkOption.NOFOLLOW_LINKS);
            if (!"root".equals(owner.getName())) {
                throw new MaintenanceException("ACADEMY_ENV_FILE_OWNER_INVALID");
            }
        } catch (IOException e) {
            throw new MaintenanceException("ACADEMY_ENV_FILE_OWNER_UNREADABLE", e);
        }
    }
    ...
    private static Map<String, String> readWithoutShell(
        ...
        try {
            ...
            throw new MaintenanceException("DATABASE_ENV_FILE_READ_FAILED", e);
        }
        ...
        for (String original : lines) {
            ...
            if (separator < 1) {
                throw new MaintenanceException("DATABASE_ENV_FILE_SYNTAX_INVALID");
            }
    ...
    private static String required(
        ...
        if (value == null || value.isBlank()) {
            throw new MaintenanceException("DATABASE_ENV_MISSING_" + name);
        }
        return value;
    }
    ...
    private static URI jdbcIdentity(
        ...
        if (!jdbcUrl.startsWith("jdbc:postgresql://")) {
            throw new MaintenanceException("DATABASE_URL_INVALID");
        }
        try {
            ...
            throw new MaintenanceException("DATABASE_URL_INVALID", e);
        }
    }
