...
class PublishPendingForecastsStalenessTest {
    ...
    @AfterEach
    void 로그_수집을_끝낸다() {
        ((Logger) LoggerFactory.getLogger(PublishPendingForecastsService.class)).detachAppender(forecastLog);
    }
    ...
    void 거슬러_보는_폭은_창_뒤로_상한만큼이다() {
        ...
        assertThat(capturedLeftBehindWindow()).isEqualTo(new LeftBehindWindow(
            NOW.minus(STALENESS).minus(ForecastPolicy.MAX_STALENESS), NOW.minus(STALENESS)));
    }
    ...
    void 창을_상한까지_넓혀도_보는_폭이_안_줄어든다() {
        ...
        assertThat(Duration.between(actual.from(), actual.until()))
            .isEqualTo(ForecastPolicy.MAX_STALENESS);
    }
    ...
    private PublishPendingForecastsService jobWith(
        Duration staleness
    ) {
    ...
    private static ListAppender<ILoggingEvent> startCapturingForecastLog() {
        ...
        appender.start();
        ((Logger) LoggerFactory.getLogger(PublishPendingForecastsService.class)).addAppender(appender);
        return appender;
    }
