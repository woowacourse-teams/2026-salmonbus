...
/**
 * 한 노선 버전·시간대에 해당하는 전체 셀 통계. 같은 통계 버전의 산출물이다.
 *
 * <p>다음 세 계산에 같은 버전의 전체 셀이 필요하므로 셀 하나만 따로 조회하지 않는다.
 *
 * <ul>
 *   <li>z화. 평균과 표준편차를 저장하지 않고 같은 세대의 행들에서 그때 유도한다
 *   <li>이웃 폴백. 해당 셀이 없으면 반경 4 안의 다른 셀이 필요하다
 *   <li>구간합. 통과할 구간의 셀들을 합쳐 읽는다
 * </ul>
 *
 * <p>승차 가능한 정류장만 포함하므로 전체 정류장 수와 행 수는 다를 수 있고 순번이 건너뛸 수 있다.
 */
public record StopDemandStatistics(
     ...
     *
     * <p>계산 범위는 대상 순번에서 예보 거리만큼 거슬러 올라간 위치부터 대상 순번까지다.
